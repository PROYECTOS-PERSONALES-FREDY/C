 ...
 if(vcomp >= 200000 && vcomp < 1000000){
	desco= (vcomp * 5)/100;
	 if (desco>=1 && desco <=250000){
	    ...
	    int total=vcomp-desco;
	   	c++;
		cout<<nombre<<" se aplica el descuento de 5% a su compra por lo tanto tiene un total a pagar de: "<<total<<""<<endl;
		}
		}else{
...
cout<<"A "<<c<<" clientes se les hace efectivo el descuento del 5%"<<endl;	
}
